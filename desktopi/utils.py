    ...
    try:
        if os_name == "Darwin":
            ...
            if desktop_env in ["gnome", "unity"]:
                ...
                args = ["gsettings", "set",
                        "org.cinnamon.desktop.background", "picture-uri", uri]
                subprocess.check_call(args)
                ...
                args = ["dbus-send", "--session", "--dest=org.kde.plasmashell", "--type=method_call",
...
def start_timed_wallpaper(image_list, seconds):
    ...
    if os_name in ["Darwin", "Linux"]:
        ...
        # TODO: ADD CODE FOR WINDOWS SERVICE
        pass
...
def stop_timed_wallpaper():
    ...
    if os_name in ["Darwin", "Linux"]:
        ...
        # TODO: WINDOWS CODE TO STOP DAEMON
        pass


# pos argument takes values:
# topleft(default), topright, bottomleft, bottomright, center
def watermark_image(image_path, text, pos="topleft", img_frac=0.20, padding=3, coords=None):
    try:
        ...
        while font.getsize(text)[0] < img_frac * base_width:
            font_size += 1
            font = ImageFont.truetype(font_path, font_size)
        ...
        watermark_coords = (padding, padding)
        if coords == None:
            if pos == "topright":
                watermark_coords = (base_width - font_width - padding, padding)
            elif pos == "bottomleft":
                watermark_coords = (padding, base_height -
                                    font_height - padding)
            elif pos == "bottomright":
                watermark_coords = (
                    base_width - font_width - padding, base_height - font_height - padding)
            elif pos == "center":
                watermark_coords = (
                    base_width / 2 - font_width / 2, base_height / 2 - font_height / 2)
        ...
        drawing.text(watermark_coords, text, fill=(
            255, 255, 255, 128), font=font)
